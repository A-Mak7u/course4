...
BATCH_DIR="outputs_runs/${TS}_auto_heavy_resume"
mkdir -p "$BATCH_DIR"
LOG_MAIN="$BATCH_DIR/batch.log"

echo "[resume] ts=$TS" | tee -a "$LOG_MAIN"
echo "[resume] root=$ROOT_DIR" | tee -a "$LOG_MAIN"
echo "[resume] python=$PY" | tee -a "$LOG_MAIN"

SPATIAL_OUT="outputs_runs/${TS}_spatial_transfer_preflight_serious_fix"
echo "[resume] stage=2 spatial_preflight_serious out=$SPATIAL_OUT" | tee -a "$LOG_MAIN"
...
echo "[resume] stage=2 done" | tee -a "$LOG_MAIN"

UNC80_OUT="outputs_runs/${TS}_saratov_uncertainty_cov80_strict"
echo "[resume] stage=3 uncertainty_cov80_strict out=$UNC80_OUT" | tee -a "$LOG_MAIN"
...
echo "[resume] stage=3 done" | tee -a "$LOG_MAIN"

UNC85_OUT="outputs_runs/${TS}_saratov_uncertainty_cov85_strict"
echo "[resume] stage=4 uncertainty_cov85_strict out=$UNC85_OUT" | tee -a "$LOG_MAIN"
"$PY" transfer/saratov_uncertainty_intervals.py \
  --input-csv final_2013_2023_T_ERA5_LST_daynight.csv \
  --device cuda \
  --n-trials 40 \
...
echo "[resume] stage=4 done" | tee -a "$LOG_MAIN"

echo "[resume] finished" | tee -a "$LOG_MAIN"
echo "[resume] outputs:" | tee -a "$LOG_MAIN"
