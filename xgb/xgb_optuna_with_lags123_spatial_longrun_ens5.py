# модификация lags123_spatial_longrun: ансамбль из 5 сидов (усреднение предсказаний), те же метрики/разбивки/графики
...
inner_train = apply_station_train_mean_feature(inner_train, inner_train, scol, target_col="T")
inner_val = apply_station_train_mean_feature(inner_val, inner_train, scol, target_col="T")
train = apply_station_train_mean_feature(train, train, scol, target_col="T")
test = apply_station_train_mean_feature(test, train, scol, target_col="T")
...
def pred_avg(X, y):
    mats = D(X, y)
    ps = [m.predict(mats) for m in models]
...
pred_train = pred_avg(train, train["T"])
pred_test  = pred_avg(test,  test["T"])
full_df = df.dropna(subset=["T"]).copy()
full_df = apply_station_train_mean_feature(full_df, train, scol, target_col="T")
pred_full = pred_avg(full_df, full_df["T"])
...
outdir = f"outputs_runs/{ts}_lags123_spatial_longrun_ens5"
...
for i,m in enumerate(models,1): m.save_model(os.path.join(outdir,f"model_{i}.json"))
...
try:
    ...
    plt.xlabel("True (test)"); plt.ylabel("Pred (test)"); plt.title("Pred vs True (test) [ens5]")
...
try:
    ...
    plt.hist(resid, bins=60); plt.xlabel("Residual (test)"); plt.ylabel("Count"); plt.title("Residuals histogram (test) [ens5]")
...
try:
    ...
    plt.xlabel("Month"); plt.ylabel("Error (Pred-True)"); plt.title("Error by month (full) [ens5]")
...
try:
    ...
    plt.plot(lims, lims); plt.xlabel("True (full)"); plt.ylabel("Pred (full)"); plt.title("Density True vs Pred (full) [ens5]")
