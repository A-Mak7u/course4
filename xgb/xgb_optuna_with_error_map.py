# карта ошибок: модель с лагами+spatial, строит карты Bias/MAE по станциям
...
# station_train_mean_T ДО split
df_nonan = df.dropna(subset=[target]).copy()
train_mean_T = df_nonan[df_nonan["year"]<=2021].groupby(scol)[target].mean().rename("station_train_mean_T")
...
outdir = f"outputs_runs/{ts}_error_map"
...
# ошибки по станциям
def save_station_errors(df_src, y_true, y_pred, path_csv):
    rows=[]
    for k,g in df_src.assign(__y=y_true,__p=y_pred).groupby(scol):
        if len(g)<5: continue
        bias=(g["__p"]-g["__y"]).mean()
        mae =np.abs(g["__p"]-g["__y"]).mean()
        rows.append(dict(station=k, n=len(g), bias=bias, mae=mae))
    pd.DataFrame(rows).to_csv(path_csv,index=False)

save_station_errors(test,  test[target],  pred_test,  os.path.join(outdir,"station_errors_test.csv"))
save_station_errors(full_df, full_df[target], pred_full, os.path.join(outdir,"station_errors_full.csv"))

# карты ошибок (scatter по координатам)
try:
    import geopandas as gpd
    import matplotlib.colors as mcolors

    def plot_station_map(csvfile, value_col, title, pngfile):
        dfm=pd.read_csv(csvfile)
        merged=pd.merge(dfm, df[[scol,latc,lonc]].drop_duplicates(), left_on="station", right_on=scol, how="left")
        plt.figure(figsize=(7,5))
        sc=plt.scatter(merged[lonc], merged[latc], c=merged[value_col], cmap="coolwarm", s=50, edgecolor="k")
        plt.colorbar(sc,label=value_col)
        plt.title(title)
        plt.tight_layout()
        plt.savefig(pngfile,dpi=160); plt.close()

    plot_station_map(os.path.join(outdir,"station_errors_test.csv"),"bias","Bias (test)",os.path.join(outdir,"map_bias_test.png"))
    plot_station_map(os.path.join(outdir,"station_errors_test.csv"),"mae","MAE (test)",os.path.join(outdir,"map_mae_test.png"))
    plot_station_map(os.path.join(outdir,"station_errors_full.csv"),"bias","Bias (full)",os.path.join(outdir,"map_bias_full.png"))
    plot_station_map(os.path.join(outdir,"station_errors_full.csv"),"mae","MAE (full)",os.path.join(outdir,"map_mae_full.png"))
