# модификация lags123_spatial: добавлена постобработка bias по станциям (средний резидуал на train)
...
def objective(trial):
    ...
    trial.set_user_attr("best_iteration", int(getattr(m, "best_iteration", 3999)) + 1)
...
metrics_train_before = pack(train[target], pred_train)
metrics_test_before  = pack(test[target],  pred_test)
metrics_full_before  = pack(full_df[target], pred_full)

train_resid = train[target].to_numpy() - pred_train
bias_map = pd.Series(train_resid, index=train.index).groupby(train[scol]).mean()
bias_map = bias_map.reindex(df[scol].unique()).fillna(0.0)

def apply_bias(src_df, base_pred):
    b = src_df[scol].map(bias_map).to_numpy()
    return base_pred + b

pred_train_after = apply_bias(train, pred_train)
pred_test_after  = apply_bias(test,  pred_test)
pred_full_after  = apply_bias(full_df, pred_full)

metrics_train_after = pack(train[target], pred_train_after)
metrics_test_after  = pack(test[target],  pred_test_after)
metrics_full_after  = pack(full_df[target], pred_full_after)

ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
outdir = f"outputs_runs/{ts}_lags123_spatial_bias"
os.makedirs(outdir, exist_ok=True)

json.dump(metrics_train_before, open(os.path.join(outdir,"metrics_train_before.json"),"w"), indent=2, ensure_ascii=False)
json.dump(metrics_test_before,  open(os.path.join(outdir,"metrics_test_before.json"), "w"), indent=2, ensure_ascii=False)
json.dump(metrics_full_before,  open(os.path.join(outdir,"metrics_full_before.json"), "w"), indent=2, ensure_ascii=False)
json.dump(metrics_train_after,  open(os.path.join(outdir,"metrics_train_after.json"),"w"), indent=2, ensure_ascii=False)
json.dump(metrics_test_after,   open(os.path.join(outdir,"metrics_test_after.json"), "w"), indent=2, ensure_ascii=False)
json.dump(metrics_full_after,   open(os.path.join(outdir,"metrics_full_after.json"), "w"), indent=2, ensure_ascii=False)
json.dump(bp,                   open(os.path.join(outdir,"params.json"),       "w"), indent=2, ensure_ascii=False)
json.dump(list(features),       open(os.path.join(outdir,"features_used.json"),"w"), indent=2, ensure_ascii=False)
...
save_group_metrics(test,  test[target],  pred_test,        "month", os.path.join(outdir,"metrics_by_month_test_before.csv"))
save_group_metrics(test,  test[target],  pred_test,        scol,    os.path.join(outdir,"metrics_by_station_test_before.csv"))
save_group_metrics(full_df, full_df[target], pred_full,    "month", os.path.join(outdir,"metrics_by_month_full_before.csv"))
save_group_metrics(full_df, full_df[target], pred_full,    scol,    os.path.join(outdir,"metrics_by_station_full_before.csv"))
save_group_metrics(test,  test[target],  pred_test_after,  "month", os.path.join(outdir,"metrics_by_month_test.csv"))
save_group_metrics(test,  test[target],  pred_test_after,  scol,    os.path.join(outdir,"metrics_by_station_test.csv"))
save_group_metrics(full_df, full_df[target], pred_full_after,"month",os.path.join(outdir,"metrics_by_month_full.csv"))
save_group_metrics(full_df, full_df[target], pred_full_after, scol,  os.path.join(outdir,"metrics_by_station_full.csv"))

try:
    plt.figure(figsize=(6,6))
    lims = [float(min(test[target].min(), pred_test_after.min())), float(max(test[target].max(), pred_test_after.max()))]
    plt.scatter(test[target], pred_test_after, s=4, alpha=0.5); plt.plot(lims, lims)
    plt.xlabel("True (test)"); plt.ylabel("Pred (test)"); plt.title("Pred vs True (test) [after]")
    plt.tight_layout(); plt.savefig(os.path.join(outdir,"scatter_pred_vs_true.png"), dpi=160); plt.close()
except Exception: pass

try:
    plt.figure(figsize=(6,4))
    resid = pred_test_after - test[target].to_numpy()
    plt.hist(resid, bins=60)
    plt.xlabel("Residual (test)"); plt.ylabel("Count"); plt.title("Residuals histogram (test) [after]")
...
try:
    ...
    for m in range(1,13):
        ...
        if len(s) >= 5:
            box.append((pred_full_after[full_df["month"]==m] - s[target].to_numpy()))
    plt.boxplot([b for b in box if len(b)>0], tick_labels=[str(i) for i in range(1,1+len(box))])
    plt.xlabel("Month"); plt.ylabel("Error (Pred-True)"); plt.title("Error by month (full) [after]")
...
try:
    ...
    x = full_df[target].to_numpy(); y = pred_full_after
    xy = np.vstack([x,y])
    try:
        z = gaussian_kde(xy)(xy); idx = z.argsort(); x, y, z = x[idx], y[idx], z[idx]
        plt.scatter(x, y, c=z, s=3)
    except Exception:
        plt.scatter(x, y, s=3, alpha=0.3)
    lims = [float(min(x.min(), y.min())), float(max(x.max(), y.max()))]
    plt.plot(lims, lims)
    plt.xlabel("True (full)"); plt.ylabel("Pred (full)"); plt.title("Density True vs Pred (full) [after]")
...
try:
    ...
    plt.scatter(test[target], pred_test, s=4, alpha=0.5); plt.plot(lims, lims)
    plt.xlabel("True (test)"); plt.ylabel("Pred (test)"); plt.title("Pred vs True (test) [before]")
    plt.tight_layout(); plt.savefig(os.path.join(outdir,"scatter_pred_vs_true_before.png"), dpi=160); plt.close()
except Exception: pass

try:
    plt.figure(figsize=(6,4))
    resid_b = pred_test - test[target].to_numpy()
    plt.hist(resid_b, bins=60)
    plt.xlabel("Residual (test)"); plt.ylabel("Count"); plt.title("Residuals histogram (test) [before]")
    plt.tight_layout(); plt.savefig(os.path.join(outdir,"residuals_hist_before.png"), dpi=160); plt.close()
except Exception: pass

try:
    plt.figure(figsize=(6,5))
    x = full_df[target].to_numpy(); y = pred_full
    plt.scatter(x, y, s=3, alpha=0.3)
    lims = [float(min(x.min(), y.min())), float(max(x.max(), y.max()))]
    plt.plot(lims, lims)
    plt.xlabel("True (full)"); plt.ylabel("Pred (full)"); plt.title("Density True vs Pred (full) [before]")
    plt.tight_layout(); plt.savefig(os.path.join(outdir,"density_true_vs_pred_full_before.png"), dpi=160); plt.close()
except Exception: pass
