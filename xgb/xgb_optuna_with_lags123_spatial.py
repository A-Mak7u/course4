...
model = xgboost.train(bp, D(train, train[target]), num_boost_round=best_rounds, verbose_eval=False)
...
save_group_metrics(test,  test[target],  pred_test,  "month",   os.path.join(outdir,"metrics_by_month_test.csv"))
save_group_metrics(test,  test[target],  pred_test,  scol,      os.path.join(outdir,"metrics_by_station_test.csv"))
save_group_metrics(full_df, full_df[target], pred_full, "month", os.path.join(outdir,"metrics_by_month_full.csv"))
save_group_metrics(full_df, full_df[target], pred_full, scol,    os.path.join(outdir,"metrics_by_station_full.csv"))
...
try:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir,"scatter_pred_vs_true.png"), dpi=160)
    plt.close()
...
try:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir,"residuals_hist.png"), dpi=160)
    plt.close()
except Exception: pass

try:
    mf = pd.read_csv(os.path.join(outdir,"metrics_by_month_full.csv"))
    mf = mf.sort_values("group")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outdir,"boxplot_error_by_month.png"), dpi=160)
    plt.close()
except Exception: pass
...
try:
    ...
    plt.plot(lims, lims)
    plt.xlabel("True (full)")
    plt.ylabel("Pred (full)")
    plt.title("Density True vs Pred (full)")
    plt.tight_layout()
    plt.savefig(os.path.join(outdir,"density_true_vs_pred_full.png"), dpi=160)
    plt.close()
except Exception: pass
