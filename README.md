...
## 7. Волгоградский датасет и transfer
...
Подтверждённый сборочный контур:
...
- собрана целевая таблица `Cod / Date / X / Y / T` для Волгограда
- `ERA5` smoke-test подтверждён файлом `data/volgograd/interim/era5_daily_yearly/volgograd_era5_daily_2014_01.csv` (`403` строк = `13 x 31`)
- `MODIS AppEEARS` probe подтверждён на реальном `results.csv`, parser уже собирает daily-таблицу `Cod / Date / LST_Day / LST_Night`
- `xgboost` на `CUDA` в новой среде работает, orchestrator готовит `full`, `fewshot_5`, `fewshot_3`

Текущее подтверждённое состояние:

- полный `ERA5 2013-2023` идёт помесячно
- большой `MODIS AppEEARS` task на `2013-2023` остаётся в `processing`
- дополнительно отправлены yearly `MODIS` tasks `2013..2023`
- bottleneck сейчас находится в приходе полного `MODIS`-набора, а не в коде модели

Итог этапа: под второй регион уже есть совместимый data-pipeline и готовый transfer-контур; финальные волгоградские метрики будут добавляться в этот же отчёт после сборки полного `Volgograd final CSV`.
...
- По Волгограду подтверждена сборка целевой таблицы, `ERA5`/`MODIS` pipeline и `CUDA`-готовый transfer-контур; финальные метрики ещё не зафиксированы, потому что полный датасет пока догружается.
