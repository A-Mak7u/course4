...
Эксперименты по восстановлению температуры `T` (`2013-2023`).
...
Итог: линейные и полиномиальные аппроксимации использовались как проверка интерпретации; рабочее качество даёт XGBoost-ветка.
...
Переход на extra-features дал основной прирост качества относительно базовой модели.
...
## 16. Сводка метрик (test 2022-2023)

| Этап | R2 | RMSE | MAE |
|---|---:|---:|---:|
| Базовая модель (`20250905_142927`) | 0.9684 | 2.0564 | 0.9750 |
| Extra-признаки (`20250914_214644_extra`) | 0.9868 | 1.3300 | 0.8492 |
| Лаги t-1 (`20250916_154740_lags`) | 0.9875 | 1.2945 | 0.7963 |
| Лаги t-1..t-3 (`20250916_163343_lags123_fix`) | 0.9896 | 1.1816 | 0.7452 |
| Лаги + spatial (`20250916_171729_lags123_spatial`) | 0.9898 | 1.1675 | 0.7189 |
